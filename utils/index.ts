...
export const isFileType = (obj: FileType): { isFile: boolean; message?: string } => {
...
export function randomize(pattern: string, length?: number, options?: RandomizeOptions): string {
...
export function generateUsername(name: string, numDigits = 4): string {
  // Remove spaces and convert to lowercase
  const cleanedName = name.replace(/\s/g, '').toLowerCase()

  // Generate random digits based on the specified amount
  const min = 10 ** (numDigits - 1)
  const max = 10 ** numDigits - 1
  const randomDigits = Math.floor(min + Math.random() * (max - min + 1))

  // Combine cleanedName with randomDigits
  const username = `${cleanedName}${randomDigits}`
